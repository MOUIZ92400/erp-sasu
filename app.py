import streamlit as st
import gspread
from google.oauth2.service_account import Credentials
import pandas as pd

# Informations de connexion intégrées
INFO = {
  "type": "service_account",
  "project_id": "erp-sasu",
  "private_key_id": "8a2d387163fe21237b1989385d37844f019e475d",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n4tw5Z7kfolqRM8u1gFku/7x95jX7+i28aS4gcKM8sfKYi22o6tx\\n-----END PRIVATE KEY-----\n",
  "client_email": "admin-sasu@erp-sasu.iam.gserviceaccount.com",
}

# Connexion à Google Sheets
try:
    scope = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive"]
    creds = Credentials.from_service_account_info(INFO, scopes=scope)
    client = gspread.authorize(creds)
    sheet = client.open("Base_Donnees_SASU").sheet1
except Exception as e:
    st.error(f"Erreur de connexion : {e}")

st.title("📊 ERP SASU Mobile")

with st.form("Saisie"):
    prod = st.text_input("Désignation Produit")
    orig = st.selectbox("Origine", ["Chine", "Maroc", "Turquie", "France"])
    prix = st.number_input("Coût HT (€)", min_value=0.0)
    
    if st.form_submit_button("🚀 Enregistrer dans le Sheets"):
        date = pd.Timestamp.now().strftime("%d/%m/%Y")
        sheet.append_row([date, prod, orig, prix])
        st.success(f"Enregistré : {prod}")
        st.balloons()
